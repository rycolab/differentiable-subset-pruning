...
class PruningConfig(FairseqDataclass):
    ...
    num_of_heads: Optional[int] = field(
        default=None,
        metadata={
            "help": "number of heads to be kept."
        },
    )
    ...
    joint_pruning: Optional[bool] = field(
        default=False,
        metadata={"help": "if set, train head importance variables and other parameters in the original model together."},
    )
    annealing: bool = field(
        default=False,
        metadata={"help": "if set, anneal the temperature of DSP."},
    )
    initial_temperature: float = field(
        default=0.1,
        metadata={"help": "initial temperature of annealing."},
    )
    ...
    # Michel Arguments
    exact_pruning: Optional[bool] = field(
        default=False,
        metadata={"help": "Compute head importance for each step"},
    )
