...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None, pruning_lr=None, joint_pruning=True):
        ...
        self.pruning_lr = pruning_lr
        self.joint_pruning = joint_pruning
    ...
    def _build_optimizer(self):
        if self.pruning_lr is None:
            ...
            params = [
                {'params': [p for n, p in self.model.named_parameters() if n != "w" and p.requires_grad]},
                {'params': [p for n, p in self.criterion.named_parameters() if p.requires_grad]},
                {
                    'params': [p for n, p in self.model.named_parameters() if n == "w" and p.requires_grad],
                    'lr': self.pruning_lr,
                }
        ...
        try:
            ...
            if not self.tpu:
                ...
                if not torch.isfinite(grad_norm).all():
                    ...
                    print("w", self.model.get_w())
                    print("w_grad", self.model.get_w().grad)
                    # raise FloatingPointError("gradients are Nan/Inf")

            with torch.autograd.profiler.record_function("optimizer"):
                # take an optimization step
                # self.model.w.grad.data.mul_(1e5)
    ...
    def prune_step(self, sample, head_mask, raise_oom=False):
        """Do forward, backward and parameter update."""
        self._set_seed()
        self.model.eval()
        self.criterion.eval()
